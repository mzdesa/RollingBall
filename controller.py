...
class BallCLF(BallCLFOrient):
    ...
    def __init__(self, observer, lyapunovBarrierList=None, trajectory=None, depthCam=None):
        ...
        #set tuning terms
        self.gamma = 0.5
        self.p = 1 #tuning for relaxation
    ...
    def eval_input(self, t):
        ...
        if V <= self.tol:
            self._u = np.zeros((3, 1))
            return self._u
        opti.subject_to(Vdot <= -self.gamma * V + delta)

        #print value of Lyapunov function
        print(V)

        #define cost
        omegaStar = self.posControl.eval_input(t)
        cost = (omegaStar - omega).T @ (omegaStar - omega) + self.p * delta**2
